...
namespace lobby
{
    public partial class frmPostPayment : Form
    {
        ...
        decimal debt;
        int numAct, reservationID, user, countryMode;
        string serie = "";
        string roomNumber;
        List<int> lsTN;
        
        public frmPostPayment(decimal balance_, int resvID_, List<int> listTrxNumber_, int userID_, string resvRoom_, int countryMode_)
        {
            InitializeComponent();
            debt = balance_;
            ...
            lsTN = listTrxNumber_;
            user = userID_;
            roomNumber = resvRoom_;
        }

        private void frmPostPayment_Load(object sender, EventArgs e)
        {
            label4.Text = "";
            List<fnGetPaymentListResult> lsPayment;
            lsPayment = hotel.fnGetPaymentList().ToList();
